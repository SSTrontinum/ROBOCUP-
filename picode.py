...
def analyse_image(image):
    ...
    ### GENERATE BLACK ###
    ######################
    if plot: mod = img.copy() # Mod is used to store the final analysed image
    if plot: contourimg = img.copy() # This will save the image that shows all the contours
    ...
    for contour in contours[1:]: # Skip first centroid
        if cv2.contourArea(contour) > 5000: # Size requirement for it to be valid
            ...
            if plot: cv2.rectangle(contourimg, (x, y), (x + w, y + h),(0, 0, 255),2)
